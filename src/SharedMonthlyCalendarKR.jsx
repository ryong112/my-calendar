    ...
    <main className="mx-auto w-full min-w-0 max-w-[2200px] overflow-x-hidden px-3 py-3 sm:px-4 lg:px-5 lg:py-3 xl:flex xl:min-h-0 xl:flex-1 xl:flex-col 2xl:px-7">
      ...
      <div className="grid w-full min-w-0 gap-3 xl:min-h-0 xl:flex-1 xl:grid-cols-[minmax(0,1fr)_320px] 2xl:grid-cols-[minmax(0,1fr)_420px]">
        <div className="w-full min-w-0 xl:flex xl:min-h-0 xl:flex-col">
          ...
          <section className="mb-2 w-full min-w-0 max-w-full shrink-0 rounded-2xl border border-slate-200 bg-white p-2.5 shadow-sm">
            <div className="flex min-w-0 flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
              ...
              <div className="flex w-full min-w-0 max-w-full flex-wrap gap-1.5 pb-1 sm:gap-2 xl:w-auto xl:pb-0"><Filter active={filter === "all"} label="전체 구분" onClick={() => setFilter("all")} />{GROUPS.map((group) => <Filter key={group.id} {...group} active={filter === group.id} label={group.name} onClick={() => setFilter(group.id)} />)}<select className="max-w-full shrink-0 rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm font-bold text-slate-700" value={typeFilter} onChange={(event) => setTypeFilter(event.target.value)} aria-label="일정 카테고리 필터"><option value="all">모든 카테고리</option>{EVENT_TYPES.map((type) => <option key={type.id} value={type.id}>{type.icon} {type.name}</option>)}</select></div>
            </div>
          </section>
        ...
        <aside className="w-full min-w-0 max-w-full xl:min-h-0"><section className="min-w-0 rounded-2xl border border-slate-200 bg-white p-3 shadow-sm sm:p-4 xl:flex xl:h-full xl:min-h-0 xl:flex-col">
          <div className="mb-4 flex min-w-0 items-center justify-between gap-2"><div className="min-w-0 flex-1"><p className="text-xs font-bold text-blue-600 xl:text-sm">선택한 날짜</p><h2 className="mt-1 truncate text-base font-black xl:text-lg 2xl:text-xl" title={formatDate(selectedDate)}>{formatDate(selectedDate)}</h2></div>{embedMode ? <a href={standaloneUrl} target="_blank" rel="noopener noreferrer" className="inline-flex min-h-8 shrink-0 items-center justify-center whitespace-nowrap rounded-lg bg-blue-600 px-2.5 py-1.5 text-[11px] font-black text-white shadow-sm transition-colors hover:bg-blue-700">일정 등록 ↗</a> : <button className={`${isAdmin ? "button-primary" : "button-disabled"} shrink-0 whitespace-nowrap !px-3`} onClick={() => openForm(selectedDate)} disabled={!isAdmin}>＋ 일정</button>}</div>
...
function MobileDay({ date, holidayNames, events, faded, today, selected, select }) {
  ...
  return <button className={`relative min-h-[54px] min-w-0 overflow-hidden border-b border-r border-slate-200 p-1 text-center ${faded ? "bg-slate-50" : "bg-white"} ${selected ? "z-10 ring-2 ring-inset ring-blue-500" : ""}`} onClick={select} title={holiday || events.map((event) => event.title).join(", ")}><span className={`mx-auto grid h-6 w-6 place-items-center rounded-full text-xs font-black ${today ? "bg-blue-600 text-white" : faded ? "text-slate-300" : holiday || date.getDay() === 0 ? "text-red-500" : date.getDay() === 6 ? "text-blue-500" : "text-slate-800"}`}>{date.getDate()}</span>{holiday && <span className="mt-0.5 block truncate text-[7px] font-black text-red-500">{holiday}</span>}<span className="mt-1 flex h-2 min-w-0 items-center justify-center gap-0.5 overflow-hidden">{events.slice(0, 3).map((event) => <i key={event.id} className={`h-1.5 w-1.5 shrink-0 rounded-full ${event.status === "cancelled" ? "opacity-30" : ""}`} style={{ backgroundColor: getGroup(event.group).color }} />)}{events.length > 3 && <b className="ml-0.5 shrink-0 text-[8px] leading-none text-slate-500">+{events.length - 3}</b>}</span></button>;
}
