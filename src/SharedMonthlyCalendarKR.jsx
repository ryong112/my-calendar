...
export default function SharedMonthlyCalendarKR() {
  ...
  const save = async (formEvent) => {
    ...
    setSaving(true);
    try {
      await addDoc(collection(db, "events"), { ...draft, dateKey: draft.startDateKey, title: draft.title.trim(), body: draft.body.trim(), location: draft.location.trim(), centerId: CENTER_ID, createdAt: serverTimestamp(), updatedAt: serverTimestamp() });
    ...
    <main className="mx-auto w-full max-w-[1600px] px-4 py-4 sm:px-6 lg:flex lg:min-h-0 lg:flex-1 lg:flex-col lg:px-6 lg:py-3">
      ...
      <div className="grid gap-4 lg:min-h-0 lg:flex-1 lg:grid-cols-[minmax(0,1fr)_300px] xl:grid-cols-[minmax(0,1fr)_340px]">
        ...
        <aside className="lg:min-h-0"><section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm lg:flex lg:h-full lg:min-h-0 lg:flex-col">
          ...
          {selectedEvents.length ? <div className="max-h-52 space-y-3 overflow-y-auto">{selectedEvents.map((event) => <Event key={event.id} event={event} admin={isAdmin} remove={() => remove(event.id)} changeStatus={(status) => changeStatus(event.id, status)} />)}</div> : <div className="rounded-xl border border-dashed border-slate-300 bg-slate-50 p-5 text-center"><p className="font-bold text-slate-600">등록된 일정이 없습니다</p><p className="mt-1 text-sm text-slate-400">{isAdmin ? "일정 버튼을 눌러 등록하세요." : "등록 즉시 실시간으로 표시됩니다."}</p></div>}
...
function Day({ date, holidayNames, events, faded, today, selected, select, add }) {
  ...
  return <button className={`min-h-28 overflow-hidden border-b border-r border-slate-200 p-2 text-left transition hover:bg-blue-50/50 lg:min-h-0 ${faded ? "bg-slate-50/80" : "bg-white"} ${selected ? "z-10 ring-2 ring-inset ring-blue-500" : ""}`} onClick={select} onDoubleClick={add}><div className="mb-1.5 flex items-center justify-between gap-1"><span className={`grid h-6 min-w-6 place-items-center rounded-full text-xs font-black ${today ? "bg-blue-600 text-white" : faded ? "text-slate-300" : holiday || date.getDay() === 0 ? "text-red-500" : date.getDay() === 6 ? "text-blue-500" : "text-slate-700"}`}>{date.getDate()}</span>{holiday && <span className="truncate text-[9px] font-bold text-red-500" title={holiday}>{holiday}</span>}</div><div className="space-y-1">{events.slice(0, 3).map((event) => { const group = getGroup(event.group); const type = getEventType(event.eventType); const priority = getPriority(event.priority); const key = toKey(date); const multiDay = event.startDateKey !== event.endDateKey; const segment = !multiDay ? "" : key === event.startDateKey ? "multi-start" : key === event.endDateKey ? "multi-end" : "multi-middle"; const recent = recentLabel(event); return <div key={event.id} className={`event-chip ${segment} ${event.status === "cancelled" ? "opacity-50 line-through" : ""} ${!multiDay && event.priority === "urgent" ? "ring-1 ring-red-400" : !multiDay && event.priority === "important" ? "ring-1 ring-amber-400" : ""}`} style={{ backgroundColor: multiDay ? group.color : group.tint, borderColor: group.color, color: multiDay ? "white" : undefined }}>{priority.icon && <span className={event.priority === "urgent" ? "mr-1 text-red-600" : "mr-1 text-amber-500"}>{priority.icon}</span>}<span className="mr-1 opacity-70">{type.icon}</span>{event.startTime && key === event.startDateKey && <span className="mr-1 opacity-75">{event.startTime}</span>}{event.title}{recent && <span className="ml-1 rounded bg-white/90 px-1 text-[8px] font-black text-blue-700 no-underline">{recent}</span>}</div>; })}{events.length > 3 && <p className="px-1 text-[10px] font-bold text-slate-400">+ {events.length - 3}개 더보기</p>}</div></button>;
}
function Event({ event, admin, remove, changeStatus }) { const group = getGroup(event.group); const type = getEventType(event.eventType); const priority = getPriority(event.priority); const status = getStatus(event.status); const recent = recentLabel(event); return <article className={`overflow-hidden rounded-xl border shadow-sm ${event.status === "cancelled" ? "opacity-65" : ""} ${event.priority === "urgent" ? "border-red-300" : event.priority === "important" ? "border-amber-300" : "border-slate-200"}`}><div className="h-1" style={{ backgroundColor: event.priority === "urgent" ? "#dc2626" : event.priority === "important" ? "#f59e0b" : group.color }} /><div className="p-3"><div className="flex justify-between gap-3"><div className="min-w-0"><div className="flex flex-wrap gap-1.5"><span className="rounded-full px-2 py-1 text-[11px] font-black" style={{ color: group.color, backgroundColor: group.tint }}>{group.name}</span><span className="rounded-full bg-slate-100 px-2 py-1 text-[11px] font-black text-slate-600">{type.icon} {type.name}</span><StatusBadge status={status} />{recent && <span className="rounded-full bg-blue-600 px-2 py-1 text-[10px] font-black text-white">{recent}</span>}{priority.id !== "normal" && <span className={`rounded-full px-2 py-1 text-[11px] font-black ${priority.id === "urgent" ? "bg-red-50 text-red-700" : "bg-amber-50 text-amber-700"}`}>{priority.icon} {priority.name}</span>}</div><h3 className={`mt-2 font-black ${event.status === "cancelled" ? "line-through" : ""}`}>{event.title}</h3></div>{admin && <button className="shrink-0 text-sm text-slate-400 hover:text-red-600" onClick={remove}>삭제</button>}</div><p className="mt-2 text-xs font-semibold text-slate-500">📅 {formatRange(event.startDateKey, event.endDateKey)}</p>{(event.startTime || event.location) && <p className="mt-2 text-sm font-semibold text-slate-600">{event.startTime && `⏱ ${event.startTime}${event.endTime ? `–${event.endTime}` : ""}`}{event.location && `  📍 ${event.location}`}</p>}{event.body && <p className="mt-2 whitespace-pre-wrap border-t pt-2 text-sm leading-5 text-slate-600">{event.body}</p>}{admin && <div className="mt-3 flex flex-wrap gap-1.5 border-t pt-3"><span className="mr-1 self-center text-[11px] font-bold text-slate-400">상태 변경</span>{STATUSES.map((item) => <button key={item.id} className={`rounded-lg px-2 py-1 text-[11px] font-black ring-1 ${event.status === item.id ? item.className : "bg-white text-slate-500 ring-slate-200"}`} onClick={() => changeStatus(item.id)}>{item.name}</button>)}</div>}</div></article>; }
function MonthItem({ event, select }) { const group = getGroup(event.group); const type = getEventType(event.eventType); const priority = getPriority(event.priority); const status = getStatus(event.status); const recent = recentLabel(event); return <button className={`flex w-full gap-3 rounded-xl border border-slate-100 p-3 text-left hover:bg-slate-50 ${event.status === "cancelled" ? "opacity-55" : ""}`} onClick={select}><div className="w-9 text-center"><b className="text-lg">{event.date.getDate()}</b><p className="text-[10px] text-slate-400">{weekday(event.date)}</p></div><div className="min-w-0 flex-1"><p className={`truncate text-sm font-bold ${event.status === "cancelled" ? "line-through" : ""}`}>{priority.icon && <span className={event.priority === "urgent" ? "mr-1 text-red-600" : "mr-1 text-amber-500"}>{priority.icon}</span>}{event.title}</p><p className="mt-1 truncate text-xs text-slate-500"><i className="mr-1 inline-block h-2 w-2 rounded-full" style={{ backgroundColor: group.color }} />{group.name} · {type.icon} {type.name}{event.startTime && ` · ${event.startTime}`}</p></div><div className="flex shrink-0 flex-col items-end gap-1"><StatusBadge status={status} />{recent && <span className="text-[9px] font-black text-blue-600">{recent}</span>}</div></button>; }
...
function FocusSummary({ color, label, event, empty, showDate = false }) { const status = event ? getStatus(event.status) : null; return <div className={`${color} rounded-2xl p-5 text-white shadow-lg`}><div className="flex items-center justify-between gap-2"><p className="text-sm font-bold opacity-80">{label}</p>{event && <span className="rounded-full bg-white/20 px-2 py-1 text-[10px] font-black">{status.name}</span>}</div>{event ? <><p className={`mt-2 truncate text-xl font-black ${event.status === "cancelled" ? "line-through opacity-60" : ""}`}>{event.title}</p><p className="mt-1 truncate text-xs opacity-80">{showDate ? `${shortDate(fromKey(event.startDateKey))} · ` : ""}{event.startTime || "종일"}{event.location ? ` · ${event.location}` : ""}</p></> : <><p className="mt-2 text-xl font-black">여유 시간</p><p className="mt-1 truncate text-xs opacity-80">{empty}</p></>}</div>; }
function StatusBadge({ status }) { return <span className={`rounded-full px-2 py-1 text-[10px] font-black ring-1 ${status.className}`}>{status.short}</span>; }
function WeekTimeline({ days, events, today, selectedDate, select }) { return <section className="week-timeline mb-3 shrink-0 overflow-x-auto rounded-2xl border border-slate-200 bg-white shadow-sm"><div className="grid min-w-[700px] grid-cols-[72px_repeat(7,minmax(0,1fr))]"><div className="flex items-center justify-center border-r border-slate-200 bg-slate-900 px-2 text-center text-xs font-black text-white">이번 주</div>{days.map((date) => { const key = toKey(date); const items = (events[key] || []).filter((item) => item.status !== "cancelled"); const first = items[0]; const group = first ? getGroup(first.group) : null; return <button key={key} className={`min-w-0 border-r border-slate-100 px-2 py-2 text-left last:border-r-0 hover:bg-blue-50 ${sameDay(date, selectedDate) ? "bg-blue-50 ring-2 ring-inset ring-blue-500" : ""}`} onClick={() => select(date)}><div className="flex items-center justify-between"><span className={`text-[10px] font-black ${date.getDay() === 0 ? "text-red-500" : date.getDay() === 6 ? "text-blue-500" : "text-slate-400"}`}>{weekday(date).slice(0, 1)}</span><span className={`grid h-5 w-5 place-items-center rounded-full text-[10px] font-black ${sameDay(date, today) ? "bg-blue-600 text-white" : "text-slate-700"}`}>{date.getDate()}</span></div><p className="mt-1 truncate text-[10px] font-bold text-slate-600">{first ? <><i className="mr-1 inline-block h-1.5 w-1.5 rounded-full" style={{ backgroundColor: group.color }} />{first.title}</> : <span className="font-medium text-slate-300">일정 없음</span>}</p>{items.length > 1 && <p className="text-[9px] font-bold text-blue-600">외 {items.length - 1}건</p>}</button>; })}</div></section>; }
