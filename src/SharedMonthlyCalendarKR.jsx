...
export default function SharedMonthlyCalendarKR() {
  ...
  return <div className="min-h-screen bg-slate-100 text-slate-950 lg:flex lg:h-dvh lg:flex-col lg:overflow-hidden">
    ...
    <main className="mx-auto w-full max-w-[1600px] px-4 py-4 sm:px-6 lg:flex lg:min-h-0 lg:flex-1 lg:flex-col lg:px-6 lg:py-3">
      <section className="summary-grid mb-2 grid shrink-0 gap-2 sm:grid-cols-3">
      ...
      <div className="grid gap-3 lg:min-h-0 lg:flex-1 lg:grid-cols-[minmax(0,1fr)_270px] xl:grid-cols-[minmax(0,1fr)_290px]">
        <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm lg:flex lg:min-h-0 lg:flex-col"><div className="overflow-x-auto lg:flex lg:min-h-0 lg:flex-1 lg:flex-col"><div className="min-w-[680px] lg:flex lg:min-h-0 lg:flex-1 lg:flex-col">
          <div className="grid grid-cols-7 border-b border-slate-200 bg-slate-50">{["일", "월", "화", "수", "목", "금", "토"].map((day, index) => <div key={day} className={`py-2.5 text-center text-base font-black ${index === 0 ? "text-red-500" : index === 6 ? "text-blue-500" : "text-slate-700"}`}>{day}</div>)}</div>
...
function Summary({ color, label, value, note }) { return <div className={`${color} rounded-xl px-4 py-3 text-white shadow-md`}><div className="flex items-center justify-between gap-3"><div><p className="text-xs font-bold opacity-85">{label}</p><b className="mt-0.5 block text-2xl font-black leading-none">{value}</b></div><span className="max-w-[50%] truncate text-[11px] font-semibold opacity-85">{note}</span></div></div>; }
function FocusSummary({ color, label, event, empty, showDate = false }) { const status = event ? getStatus(event.status) : null; return <div className={`${color} rounded-xl px-4 py-3 text-white shadow-md`}><div className="flex items-center justify-between gap-3"><div className="min-w-0"><p className="text-xs font-bold opacity-85">{label}</p>{event ? <p className={`mt-0.5 truncate text-lg font-black leading-tight ${event.status === "cancelled" ? "line-through opacity-60" : ""}`}>{event.title}</p> : <p className="mt-0.5 text-lg font-black leading-tight">여유 시간</p>}<p className="mt-0.5 truncate text-[10px] font-semibold opacity-85">{event ? <>{showDate ? `${shortDate(fromKey(event.startDateKey))} · ` : ""}{event.startTime || "종일"}{event.location ? ` · ${event.location}` : ""}</> : empty}</p></div>{event && <span className="shrink-0 rounded-full bg-white/20 px-2 py-1 text-[10px] font-black">{status.name}</span>}</div></div>; }
